import { readdirSync, existsSync, mkdirSync, writeFileSync, statSync } from 'fs'

const createJson = function(folderName) {

    const keyArr = new Set(...[
        [
            ...readdirSync(folderName),
            ...(existsSync(`${folderName}-lowres`) ? readdirSync(`${folderName}-lowres`) : [])
        ].filter(x => x != '.DS_Store')
        .map(x => x.replace(".png", "")) // Remove file extension
    ])

    let retArr = []

    for (const key of keyArr) {
        let lowresExists = existsSync(`${folderName}-lowres/${key}.png`)
        let pngExists = existsSync(`${folderName}/${key}.png`)
        let folderExists = existsSync(`${folderName}/${key}/`) && statSync(`${folderName}/${key}/`).isDirectory()
        
        if (folderExists) {
            const folderContents = readdirSync(`${folderName}/${key}`)
            let images = []

            for (const folderItem of folderContents) {
                if (folderItem.endsWith("_dark.png")) continue
                if (!folderItem.endsWith(".png")) continue
                const baseName = folderItem.replace(".png", "")
                images.push({
                    id: baseName,
                    dark: folderContents.includes(`${baseName}_dark.png`)
                })
            }

            retArr.push({
                key: key,
                count: images.length,
                dark: images.filter(x => x.dark).length > 0,
                index: images
            })
        }

        else if (pngExists || lowresExists) {
            retArr.push({
                key: key,
                count: 1,
                dark: false,
                index: [{
                    id: 0,
                    dark: false
                }]
            })
        }
    }

    return retArr;
}

if (!existsSync('out')) mkdirSync('out')
writeFileSync('out/main.json', JSON.stringify(createJson('device')))
writeFileSync('out/logos.json', JSON.stringify(createJson('images')))